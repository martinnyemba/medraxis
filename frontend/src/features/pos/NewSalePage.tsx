...
export function NewSalePage() {
  ...
  // Default to the first available location once the list loads.
  React.useEffect(() => {
  ...
  return (
    <div>
      ...
      <PageHeader title="New sale" description="Search products and build the cart." />
    ...
    <Card>
      ...
      <CardContent>
        ...
        {isFetching && !data ? (
            ...
            {data.results.map((p) => (
              <li key={p.id} className="flex items-center justify-between gap-3 py-2.5">
                ...
                <div className="flex items-center gap-2">
                  {p.quantity_on_hand <= 0 && <Badge variant="warning">No stock</Badge>}
          ...
          <p className="py-6 text-center text-sm text-muted-foreground">
            {searchInput ? "No products found." : "Search to add products to the cart."}
          </p>
        )}
      </CardContent>
    </Card>
